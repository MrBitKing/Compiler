...
Buffer* b_allocate(short init_capacity, char inc_factor, char o_mode) {
	...
	Buffer* buffer = (Buffer*)calloc(1, sizeof(Buffer)); /*Allocate buffer Desciptor*/

	if (init_capacity < 0) return NULL;
	if ((init_capacity >= 0) && (init_capacity < SHRT_MAX))  /* Check valid initial capacity */
		buffer->cb_head = (char*)malloc((init_capacity)*(sizeof(char))); /*Allocate dynamic buffer */

	if (o_mode == 'f' || (unsigned char)inc_factor == 0) {  /*fixed mode wide inc factor 0*/
		buffer->mode = 0;
		buffer->inc_factor = 0;
	}

	else if (o_mode == 'f' && (unsigned char)inc_factor != 0) { /*set fixed mode when inc factor !=0*/
	...
	else if (o_mode == 'm' && (unsigned char)(inc_factor >= 1 && (unsigned char)inc_factor <= 100)) {  /*set multi mode*/
	...
	buffer->capacity = init_capacity;  /*Set capacity to initiale capcaity*/
	return buffer;
}
...
pBuffer b_addc(pBuffer const pBD, char symbol) {
	...
	if (!pBD->cb_head || !pBD) /*Check if error*/
		return NULL;
	...
	if (!b_isfull(pBD)) {  /*Just add if array not full*/
	...
	if ((short)(pBD->addc_offset*sizeof(char)) == pBD->capacity) {  /*Array full ?*/
		...
		if (pBD->mode == 0) { /*Return if mode 0*/
			...
			if (new_increment <= 0) {  /*Increment < 0 ? */
				new_increment = 1;
			}
			if (!new_capacity)  /* Set to max*/
				new_capacity = (SHRT_MAX - 1);

			else if (pBD->capacity >= new_increment && pBD->capacity < SHRT_MAX - 1) { /* If out of range, set to max */
				new_capacity = (SHRT_MAX - 1);
			}
		}
...
short b_mark(pBuffer const pBD, short mark) {
	...
	if (mark <= pBD->addc_offset)   /*mark within reosonable range ?*/
		pBD->markc_offset = mark;

	return pBD->markc_offset;  /* current buffer mark_offset*/
}
...
int b_load(FILE * const fi, Buffer * const pBD) {
	...
	while ((character = (char)fgetc(fi)) != EOF) {  /*Is it the end of file yet ?*/
...
int b_print(Buffer* const pBD) {
	...
	//pBD->getc_offset = 0; not necessary
	while (1) {				/*Infinite loop*/
		character = b_getc(pBD);		/*get char*/
		if ((b_eob(pBD))) {     /*BREAK IF EOF*/
			break;
		}
...
Buffer * b_compact(Buffer * const pBD, char symbol) {
	...
	temp_cb_head = realloc(pBD->cb_head, (unsigned short)newCapacity);  /*Set new capacity if valid */

	if (temp_cb_head == NULL)
		return NULL;

	if (temp_cb_head != pBD->cb_head) /*Set falg if memory location changed*/
		pBD->r_flag = SET_R_FLAG;
...
char * b_location(Buffer* const pBD, short loc_offset) {
	...
	if ((loc_offset >= 0) && (loc_offset < pBD->addc_offset))
		character = &(pBD->cb_head[loc_offset]);/*Return loc_offset if in range*/
